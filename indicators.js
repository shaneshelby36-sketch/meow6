...
class OrderBook {
  ...
  spread() {
    ...
    const ba = this.bestAsk();
    if (bb == null || ba == null) return null;
    return { absolute: ba - bb, percent: ((ba - bb) / ((ba + bb) / 2)) * 100 };
  }

  // Sums size within `depthPct` percent of the mid price on each side,
  // then reports imbalance in [-1, 1] (positive = more buy-side pressure)
  imbalance(depthPct = 0.5) {
    ...
    if (total === 0) return { ratio: 0, bidVolume: 0, askVolume: 0 };
    return {
      ratio: (bidVol - askVol) / total, // -1..1
      bidVolume: bidVol,
      askVolume: askVol,
    };
  }

  // Total resting size within `depthPct` of mid, both sides — a liquidity proxy
